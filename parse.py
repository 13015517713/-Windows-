...
# 解析json并画图
def getData():
    fp = open('D:/文件/git项目/GoProject/images/instance_11_job_95482325353/instance_11_job_95482325353.json', 'r')
    data = json.load(fp)
    # 按照starttime排序
    ...
    # for i,stime in enumerate(sortedData):
    #     metric = time2Data[stime]
    #     writer.writerow([metric['start_time'],metric['average_usage']['cpus']])
    for i in newY:
        writer.writerow([i])
...
# 把预测的画到一个图中
def delCpu2Predict():
    filepath = 'D:/文件/git项目/GoProject/cpu2predict.csv'
...
# 给一行画图
def drawOne():
    filepath = 'C:/Users/24302/Desktop/毕设安排/项目相关/画图等代码/cpu_ave.csv'
    reader = csv.reader(open(filepath,"r") )
    Y = []
    for i in reader:
        Y.append(float(i[0]))
    print(np.max([i for i in Y if i<=0.0035] ) )
    print(np.min([i for i in Y if i<=0.0035] ) )
    print(np.mean([i for i in Y if i<=0.0035] ) )
    print(np.std([i for i in Y if i<=0.0035]  ) )
    print(np.var([i for i in Y if i<=0.0035]  ) )
